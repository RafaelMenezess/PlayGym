...
class AcademyCalendar extends React.Component {
    state = {
    ...
    componentDidMount() {
        // fetch('http://localhost:8000/api/student_schedule/')
        ...
        //         console.log(error);
        //     });
        fetch("http://localhost:8000/api/schedule_config/")
            .then(response => response.json())
            .then(json =>
                this.setState({
                    academyTimeOpen: json[0].SCHEDULE_CONFIG_PANDEMIC_SCHEDULES_OPEN,
                    academyTimeClose: json[0].SCHEDULE_CONFIG_PANDEMIC_SCHEDULES_CLOSE
                }, () => { console.log(this.state.academyTimeOpen) })
            )
            .catch(function (error) {
                console.log(error);
            });
    ...
    pushSchedule = async (token, selectInfo) => {
        await fetch("http://localhost:8000/api/student_schedule/", {
            method: 'POST',
            headers: {
